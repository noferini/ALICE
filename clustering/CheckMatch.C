...
Float_t DeltaZ[100];
Int_t nExtStrip;
Int_t channelExt[100];
Float_t deltaXext[100];
Float_t deltaZext[100];
Float_t deltaXextA[100];
Float_t deltaZextA[100];
Float_t deltaXextB[100];
Float_t deltaZextB[100];
...
//Bool_t CheckMatch(const char *lista="wn.xml",Bool_t kGRID=1) //per prendere dalla grid;
Bool_t CheckMatch(const char *lista="lista",Bool_t kGRID=0) // da locale
{
  ...
  T->Branch("nExtStrip",&nExtStrip,"nExtStrip/I");
  T->Branch("channelExt",channelExt,"channelExt[nExtStrip]/I");
  T->Branch("deltaXext",deltaXext,"deltaXext[nExtStrip]/F");
  T->Branch("deltaZext",deltaZext,"deltaZext[nExtStrip]/F");
  T->Branch("deltaXextA",deltaXextA,"deltaXextA[nExtStrip]/F");
  T->Branch("deltaZextA",deltaZextA,"deltaZextA[nExtStrip]/F");
  T->Branch("deltaXextB",deltaXextB,"deltaXextB[nExtStrip]/F");
  T->Branch("deltaZextB",deltaZextB,"deltaZextB[nExtStrip]/F");
...
Bool_t CheckSingle(const char* esdFileName,Bool_t kGRID){
  ...
  for(Int_t ie=0;ie < nev;ie++){
	...
	for(int im=cl->GetNMatchableTracks();im--;){ //o così o da n-1 a 0 //for(int im=cl->GetNMatchableTracks();im>0;im--) non andava bene perchè non prendeva mai lo 0  
      ...
      Double_t pos[3];
      Float_t posF[3];
      const AliExternalTrackParam* trkExt = track->GetOuterParam();

      nExtStrip = 0;
      Float_t rTOFused=373;
      AliExternalTrackParam trk(*trkExt);
      Int_t value = trk.PropagateTo(rTOFused,mag);
      //   printf("propto = %i\n",value);
      if(!value) continue;


      Float_t posPG[3],posRG[3],posFpg[3];
      Int_t detId[5];

      detId[0]=AliTOFGeometry::GetSector(posF);  
      detId[1]=AliTOFGeometry::GetPlate(posF);  
      detId[2]=AliTOFGeometry::GetStrip(posF);
      detId[3]=AliTOFGeometry::GetPadZ(posF);
      detId[4]=AliTOFGeometry::GetPadX(posF);

      posFpg[0] = posF[0]-posRG[0]+posPG[0];
      posFpg[1] = posF[1]-posRG[1]+posPG[1];
      posFpg[2] = posF[2]-posRG[2]+posPG[2];

      while(rTOFused < 400){
	if(detId[4] != -1){
	  // convert from real geom to pure geom
	  AliTOFGeometry::GetPos(detId,posRG);
	  AliTOFGeometry::GetPosPar(detId,posPG);
	  
	  // 	printf("r=%f -- (%i,%i,%i,%i,%i) %f %f %f",rTOFused,detId[0],detId[1],detId[2],detId[3],detId[4],posRG[0],posRG[1],posRG[2]);
	  // 	printf(" - %f %f %f\n",posPG[0],posPG[1],posPG[2]);
	  
	  if(TMath::Abs(AliTOFGeometry::GetPadDy(posFpg)+0.5) < 0.01){
	    deltaXextB[nExtStrip] = AliTOFGeometry::GetPadDx(posFpg);
	    deltaZextB[nExtStrip] = AliTOFGeometry::GetPadDz(posFpg);	    
	  }
	  if(TMath::Abs(AliTOFGeometry::GetPadDy(posFpg)-0.5) < 0.01){
	    deltaXextA[nExtStrip] = AliTOFGeometry::GetPadDx(posFpg);
	    deltaZextA[nExtStrip] = AliTOFGeometry::GetPadDz(posFpg);	    
	  }

	  if(TMath::Abs(AliTOFGeometry::GetPadDy(posFpg)) < 0.01){
	    // swicth on all the empty channels
	    AliTOFGeometry::GetDetID(posFpg,detId);
	    
	    printf("%f (%i) %f\n",rTOFused,AliTOFGeometry::GetIndex(detId)/96,AliTOFGeometry::GetPadDy(posFpg));
	    
	    channelExt[nExtStrip] = AliTOFGeometry::GetIndex(detId);
	    deltaXext[nExtStrip] = AliTOFGeometry::GetPadDx(posFpg);
	    deltaZext[nExtStrip] = AliTOFGeometry::GetPadDz(posFpg);
