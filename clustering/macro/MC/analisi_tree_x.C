...
hprof1  = new TProfile("hprof1","Profile t1-t_exp_pi vs dx1",26, -2.,2.);
hprof1corr  = new TProfile("hprof1corr","Profile t1-t_exp_pi corr vs dx1",26, -2.,2.);
hprof2  = new TProfile("hprof2","Profile t2-t_exp_pi vs dx2",26,-2.,2.);
hprof2corr  = new TProfile("hprof2corr","Profile t2-t_exp_pi corr vs dx2",26, -2.,2..);
...
//              hresdist2ricen->GetXaxis()->SetTitle("sqrt(Dx2ricentrato^2+Dz2^2) (cm)");
             ...
             // Float_t tw1=tempo[0]- gtime;
              ...
              ht2_texp->Fill(tw2);
              ht2_texp->GetXaxis()->SetTitle("t_{2} - t_{geant} (ps)");
              
              h2tw1tw2->Fill(tw1, tw2);
              h2tw1tw2->GetXaxis()->SetTitle("t_{1} - t_{geant}  (ps)");
              h2tw1tw2->GetYaxis()->SetTitle("t_{2} - t_{geant}  (ps)");
              ...
              //riempio istogrammma, poi lo fitto fuori dal loop
               ...
               hprof1->GetYaxis()->SetTitle("t_{1}-t_{geant} (ps)");
              
              
               hprof2->Fill(posx,tw2 ,1);
               hprof2->GetXaxis()->SetTitle("posx1 (cm)");
               hprof2->GetYaxis()->SetTitle("t_{2}-t_t_{geant} (ps)");
    ...
    for (Int_t nu=0 ; nu<nentries ; nu++)
    {
        ...
        if(TMath::Abs(dch) == 1 /*&& TMath::Abs(tempo[0]-exp_time_pi[0])<800.*/ /* per avere circa 3 sigma che sia un pi*/ && TMath::Abs(tempo[0] - tempo[1])<470.)
        {
            ...
            hprof1corr->GetYaxis()->SetTitle("t1_corr=t1-tgeant corr(ps)");
            
            hprof2corr->Fill(posx2,tw2corr,1);
            hprof2corr->GetXaxis()->SetTitle("Dx2 (cm)");
            hprof2corr->GetYaxis()->SetTitle("t2_corr=t2-tgeant corr(ps)");
    ...
    TFile *fo2 = new TFile("output_x.root","RECREATE");// DATI VERI
