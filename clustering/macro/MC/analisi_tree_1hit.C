...
hprofx  = new TProfile("hprofx","Profile t1-t_exp_pi vs dx1",26, -2.,2.);
hprofxcorr  = new TProfile("hprofxcorr","Profile t1-t_exp_pi corr vs dx1",26, -2.,2.);
    
hprofz  = new TProfile("hprofz","Profile t1-t_exp_pi vs dz1",26, -3.,3.);
    
hprofd=new TProfile("hprofd","Profile t1-t_exp_pi vs d",26, 0.,4.);
...
hprofdeff=new TProfile("hprofdeff","Profile t1-t_exp_pi vs deff",50, -3.,10.);
...
//              pzt->GetYaxis()->SetTitle("Dz1 (cm)");
              ...
              Float_t tw1=tempo[0]- StartTime - gtime[0] + interactiontime;
...
//               hprofz->GetYaxis()->SetTitle("t_{1}-t_{exp #pi} (ps)");
                ...
                hprofd->GetYaxis()->SetTitle("t_{1}-t_{geant} (ps)");
        ...
        if(ncluster == 1)
        {
            if(impulso_trasv>0.8 && impulso_trasv<1.) // serve per gli exp time
            {
                //if(TMath::Abs(tempo[0]-exp_time_pi[0])<800./* per avere circa 3 sigma che sia un pi*/ )
                    ...
                    Float_t tw1tot=tempo[0]- gtime[0];
        ...
        if(ncluster == 1)
        {
            ...
            //    if(TMath::Abs(tempo[0]-exp_time_pi[0])<800./* per avere circa 3 sigma che sia un pi*/ )
                    ...
                    if(TMath::Abs(DeltaX[0])<1.5)
                    {
                        ...
                        ht1_texptot->GetXaxis()->SetTitle("t_{1} - t_{geant} (ps)");

                        Float_t tw1corrtw=tw1tot-(offset_tw + p0 *TMath::Min(deff,1.75));//Min(x,1.75) , l'1.75 è stato scelto guardando il fit, come meglio sembrava interploare. Per ora, per quanto ne so, è solo una coincidenza che coincida con pad z
...
//    hprofxcorr->Fit("f1c","R");
    ...
    TFile *fo2 = new TFile("output_1hit.root","RECREATE");
