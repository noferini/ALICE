...
char name[300];
  ...
  else
  {
    ...
    TAlienCollection *myCollection = TAlienCollection::Open(lista);
    if (!myCollection)
    {
...
// check the content of the ESD
  ...
  for(Int_t ie=0;ie < nev;ie++)
  {
    ...
    if(! esd->GetVertex()) esd->ResetStdContent(),continue;// una volta fatto il connect manda un flag ; siccome qua c'era un continue(non si arriva in fondo al ciclo) bisogna resettarlo altrimenti lo trova già attivo.

    tofcl = esd->GetESDTOFClusters(); // AliESDTOFCluster *cltof = tofcl->At(i);
    if(tofcl->GetEntries() == 0) esd->ResetStdContent(),continue;
